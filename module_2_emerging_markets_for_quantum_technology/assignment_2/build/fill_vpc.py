...
BOXES = {
    ...
    'Pains': T(
        ...
        "is the injury."),
    'Value Proposition': T(
        "We tell contractors what lies under the ground on the sites where radar gives "
        "up. Stated honestly: we are slower per metre and coarser in resolution than "
        "radar in good conditions, so we are the second pass on hard ground, not a "
        "replacement for the first. Claiming more would put crews at risk, which is the "
        "point at which responsible innovation stops being a seminar topic and starts "
        "being a design constraint (Stilgoe, Owen and Macnaghten, 2013)."),
    'Products and Features': T(
        "A survey service rather than an instrument sale: a trolley or vehicle-mounted "
        "quantum gravity gradiometer, run by our own crews as a targeted second pass on "
        "the sites the manager already worries about. The deliverable is a "
        "depth-resolved density map with stated uncertainty on position and depth, "
        "issued in GIS and BIM formats and cross-referenced to PAS 128 quality levels."),
    'Pain Relievers': T(
    ...
    'Gain Creators': T(
        "Priced by the day against the 29-times multiplier on a single strike, the "
        "arithmetic is one a buyer can run themselves. We report outcomes on surveyed "
        "sites against the published national baseline, so the claim can be checked "
        "rather than believed, and data is returned NUAR-ready, which turns a survey "
        "cost into an asset the owner keeps."),
}
