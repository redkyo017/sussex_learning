...
Allowance 1500 (500 per item). Measured by `build/wordcount.py`, which diffs the built document against the blank templates: **1747 words — MON 613, VPC 542, BMC 592 (+16%)**; the Part 2 orientation sentence and the bold "Opportunity n — <name>" card headings added 2026-08-23 keep the three scorecards reading as three cards rather than a copy-paste. Template prompt text, checkbox labels, the title block and the reference list are excluded.
