...
in the question. I compute $Z_1\,Z_2\,\mathrm{CX}_{2,1}\,\mathrm{CX}_{2,3}\,H_2\ket{0,0,0}$
by applying the gates right-to-left, one at a time.
...
I compute $H_2\!\left(\tfrac{1}{2}\ket{\Phi^+}+\tfrac{1}{2}\ket{\Phi^-}
+\tfrac{1}{2}\ket{\Psi^+}+\tfrac{1}{2}\ket{\Psi^-}\right)$
by first simplifying the Bell-state mixture, then applying $H_2$.
...
The first qubit is measured in the $\{\ket{0},\ket{1}\}$ basis and the outcome $\ket{0}$ is obtained.
...
Consider $\ket{\Psi^-}=\dfrac{1}{\sqrt{2}}\ket{0,1}-\dfrac{1}{\sqrt{2}}\ket{1,0}$.
The first qubit is measured in the $\{\ket{+},\ket{-}\}$ basis and the outcome $\ket{-}$ is obtained.

\medskip
I extract the (unnormalised) state of qubit 2 by computing
${}_{1}\!\bra{-}\ket{\Psi^-}$.
...
Consider the same state $\ket{\Psi^-}$. The first qubit is now measured in the
$\{\ket{+i},\ket{-i}\}$ basis and the outcome $\ket{+i}$ is obtained, where
...
I compute the $2\times2$ matrix for
$U(\theta,\phi)=R_z(-\phi)\,R_y(\theta)\,R_z(\phi)$ by multiplying the three matrices
in right-to-left order. The given definitions are:
...
The four Bell states form an orthonormal basis. I compute the probability of each
outcome when measuring $\ket{+i,+i}$ in this Bell basis,
...
I use the state computed in Question~9:
$\ket{\psi_0}=\tfrac{1}{2}(\ket{000}+\ket{010}+\ket{100}+\ket{111})$,
and the result that $P(\text{qubit 3}=\ket{0})=\tfrac{3}{4}$.
